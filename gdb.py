...
def build_pretty_printer():
  ...
  pp.add_printer('bool', '^NIM_BOOL$', NimBoolPrinter)
  pp.add_printer('string', '^NimStringDesc \*$', NimStringPrinter)
  pp.add_printer('Rope', '^tyObject_RopeObj__([A-Za-z0-9]*) \*$', NimRopePrinter)
  pp.add_printer('enum', '^tyEnum_([A-Za-z0-9]+)__([A-Za-z0-9]*)$', NimEnumPrinter)
  pp.add_printer('set', '^tySet_tyEnum_([A-Za-z0-9]+)__([A-Za-z0-9]*)$', NimSetPrinter)
  pp.add_printer('HashSet', '^tyObject_(HashSet)__([A-Za-z0-9]*)$', NimHashSetPrinter)
  pp.add_printer('seq', '^tySequence_\w* \*$', NimSeqPrinter)
  pp.add_printer('array', '^tyArray_\w*$', NimArrayPrinter)
  pp.add_printer('StringTable', '^tyObject_(StringTableObj)__([A-Za-z0-9]*)(:? \*)?$', NimStringTablePrinter)
  pp.add_printer('Table', '^tyObject_(Table)__([A-Za-z0-9]*)(:? \*)?$', NimTablePrinter)
  pp.add_printer('object', '^tyObject_([A-Za-z0-9]+)__(_?[A-Za-z0-9]*)(:? \*)?$', NimObjectPrinter)
  return pp
...
class NimXMethodMatcher:
  name = "NimXMethodMatcher"
  def __init__(self):
    self.enabled = True
  def match(self, class_type, method_name):
    raise NotImplementedError("NimXMethodMatcher match")
...
class NimTypeRecognizer:
  ...
  def recognize(self, type_obj):
    ...
    #   if target_type.name:
    #     tname = nim_gdb.debuggerTypeNameFromMangled(target_type.name)
    #     # visualize 'string' as non pointer type (unpack pointer type).
    #     # if target_type.name == "NimStringDesc":
    #     #   tname = target_type.name # could also just return 'string'
    #     # else:
    ...
    # if tname:
    #   result = nim_gdb.debuggerTypeNameFromMangled(tname)
    #   if result:
    #     return result
...
class Nim(object):
  ...
  def to_string(self):
    ...
    return "None"
  def display_hint(self):
    return "None" #gdb.parse_and_eval('debuggerHint("%s")' % self.val.type.name)
  def children(self):
    yield ('config', 'Field1')

# register globally
# gdb.types.register_type_printer(None, NimTypePrinter)
# gdb.xmethod.register_xmethod_matcher(None, NimXMethodMatcher) # XXX: this is the API but the API is new and it's not working
# gdb.xmethods.insert(0, NimXMethodMatcher) # XXX: this is a workaround
# gdb.pretty_printers.insert(0, Nim)
...
# MIEcho("-data-evaluate-expression", "string")
